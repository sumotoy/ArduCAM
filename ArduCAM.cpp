...
void ArduCAM::begin(bool spiInit,bool wireInit)
{
	if (wireInit){
	#if defined(__AVR__) || defined(ESP8266) || defined(TEENSYDUINO)
		Wire.begin();
	#elif defined(__arm__) && !defined(TEENSYDUINO)//DUE?
		Wire1.begin();
	#else
		Wire.begin();
	#endif
...
#if !defined(SPI_HAS_TRANSACTION)//old SPI lib
...
byte ArduCAM::rdSensorReg8_8(uint8_t regID, uint8_t* regDat)
{
	...
	if (Wire.available()) *regDat = Wire.read(); 	
	delay(1);
	return(1);
}
